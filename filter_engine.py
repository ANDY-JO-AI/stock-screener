...
def load_corp_code_map():
    """DART 전종목 기업코드 XML → {종목코드: corp_code}"""
    try:
    ...
    for market in ['KOSDAQ', 'KOSPI']:
        ...
        for _, row in df.iterrows():
            ...
            if marcap < 150_0000_0000:
                logs.append((code, name, "L0: 시총 150억 미달")); continue
            # 시총 상한 (코스닥 기준)
            if market == 'KOSDAQ':
                ...
                limit = 700_0000_0000
            if marcap > limit:
                logs.append((code, name, f"L0: 시총 {round(marcap/1e8)}억 상한 초과")); continue
            # 우선주/스팩/중국 제외
            if any(kw in name for kw in ['우','스팩','SPAC','우선','홀딩스우','차이나','중국']):
                logs.append((code, name, "L0: 우선주/스팩/중국")); continue
            results.append({
                '종목코드': code, '종목명': name,
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            if dp is None or len(dp) < 20:
                logs.append((code, name, "L1: 주가 데이터 부족")); continue
            low_52  = dp['Low'].min()
            cur_prc = dp['Close'].iloc[-1]
            ratio   = 1.5 if market == 'KOSPI' else 1.7
            if cur_prc > low_52 * ratio:
                logs.append((code, name, f"L1: 현재가 {cur_prc:,} > 저가×{ratio} {round(low_52*ratio):,}")); continue
        except:
            logs.append((code, name, "L1: 주가 조회 오류")); continue
        ...
        if corp:
            for yr in [TODAY.year-3, TODAY.year-2, TODAY.year-1]:
                ...
                if pc: paid_in_cap = pc

            # 3년 연속 영업손실
            if all(op is not None and op <= 0 for op in op_list):
                logs.append((code, name, "L1: 3년 연속 영업손실")); fail=True

            # 최근 연도 부채비율
            if not fail and debt_list[-1] and equity_list[-1] and equity_list[-1] > 0:
                dr = debt_list[-1] / equity_list[-1] * 100
                if dr >= 100:
                    logs.append((code, name, f"L1: 부채비율 {round(dr)}%")); fail=True
            ...
            if not fail and paid_in_cap and equity_list[-1] is not None and paid_in_cap > 0:
                ...
                if erosion >= 50:
                    logs.append((code, name, f"L1: 자본잠식 {round(erosion)}%")); fail=True
        ...
        time.sleep(0.3)
        if good_count >= 4:
            logs.append((code, name, f"L1: 호재뉴스 {good_count}건 (계약금액 미확인)"))
            continue
...
def layer2_finance_score(df):
    for idx, row in df.iterrows():
        ...
        rev = None
        if corp:
            fin_latest = get_dart_financials(corp, TODAY.year-1)
            time.sleep(0.2)
            rev = extract_amount(fin_latest, "매출액")
            if rev is None:
                rev = extract_amount(fin_latest, "수익(매출액)")
        if rev and marcap > 0:
        ...
        if corp:
            ...
            time.sleep(0.2)
            retained = extract_amount(fin_l, "이익잉여금")
            paid_cap = row.get('납입자본금')
            if retained and paid_cap and paid_cap > 0:
                reserve_ratio = retained / paid_cap * 100
                if reserve_ratio >= 500:   score += 2; detail.append("적립금≥500%+2")
                elif reserve_ratio >= 300: score += 1; detail.append("적립금≥300%+1")

        # ROE
        if eq_list and eq_list[-1] and eq_list[-1] > 0:
            net_income = None
            if corp:
                fin_l2 = get_dart_financials(corp, TODAY.year-1)
                time.sleep(0.2)
                net_income = extract_amount(fin_l2, "당기순이익")
            if net_income and net_income / eq_list[-1] * 100 >= 10:
                score += 1; detail.append("ROE≥10%+1")

        # 배당 이력
        if corp:
            div_data = dart_get("alotMatter.json", {
                "corp_code":  corp,
                "bgn_de":     TWO_YRS.replace('-',''),
                "end_de":     TODAY_STR.replace('-',''),
            })
            time.sleep(0.2)
            if div_data.get("list"):
...
def layer3_liquidity(df):
    ...
    for _, row in df.iterrows():
        ...
        dp = row.get('_dp')
        if dp is None:
            logs.append((code, name, "L3: 가격데이터 없음")); continue

        try:
            dp3 = fdr.DataReader(code, THREE_YRS, TODAY_STR)
            dp3['거래대금'] = dp3['Close'] * dp3['Volume']
        except:
            logs.append((code, name, "L3: 3년 데이터 조회 실패")); continue

        peak_vol   = dp3['거래대금'].max()
        recent_avg = dp3['거래대금'].tail(10).mean()

        # 필수: 3년내 100억 이상 거래일 존재
        if peak_vol < 100_0000_0000:
            logs.append((code, name, f"L3: 최대거래대금 {round(peak_vol/1e8,1)}억 미달")); continue
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if not items:
                ...
                continue
            top_ratio = float(items[0].get('trmend_posesn_stock_co','0').replace(',','') or 0)
            tot_stock = float(items[0].get('trmend_tot_stock_co','1').replace(',','') or 1)
            ratio = round(top_ratio / tot_stock * 100, 1) if tot_stock > 0 else 0
...
def layer5_theme(df):
    for idx, row in df.iterrows():
        ...
        policy    = [t for t in matched if "방산" in t or "에너지" in t or "AI" in t]
        seasonal  = [t for t in matched if "계절" in t or "기후" in t]
        social    = [t for t in matched if "저출산" in t or "복지" in t or "대북" in t]

        score += min(len(political)*2, 4)
        score += min(len(policy)*2, 4)
        score += min(len(seasonal)*1, 2)
        score += min(len(social)*1, 2)
        ...
        try:
            ...
            if max_r >= 200: score += 2; matched.append("📈5년급등이력")
        except:
            pass
    ...
    for idx, row in df.iterrows():
        ...
        if dp is not None and len(dp) >= 20:
            ...
            if vol_60 > 0:
                ...
                elif vr >= 1.5: score += 1; signals.append(f"거래량{round(vr,1)}배소폭")
            ...
            if cur_prc and low_52 and low_52 > 0:
                prx = cur_prc / low_52
                if prx <= 1.3:   score += 2; signals.append(f"저가×{round(prx,2)}배(근접)")
                elif prx <= 1.5: score += 1; signals.append(f"저가×{round(prx,2)}배")
            ...
            # 음봉 반전 패턴 없음 (대형 음봉 = 전일 대비 -5% 이상)
            last_chg = (dp['Close'].iloc[-1] - dp['Close'].iloc[-2]) / dp['Close'].iloc[-2] * 100 if len(dp) >= 2 else 0
            if last_chg <= -5:
                score -= 2; signals.append("⚠️대형음봉주의")
...
def run_pipeline(progress_callback=None):
    ...
    log(f"  → {len(corp_map)}개 기업코드 로드 완료")
